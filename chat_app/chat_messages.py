...
class ChatMessages(tk.Frame):
    def __init__(self, frame, discussion_list=None):
        ...
        self.chat_messages = self.load_data('D:\\chat_app\\resources\\messages.json')
    ...
    def send_message(self):
        #selected_index = self.discussion_list.listbox_discussions.selection()
        messages_data = self.load_data('D:\\chat_app\\resources\\messages.json')
        ...
        for user_id in messages_data:
            if user_id == self.contact_id:
                contact_messages_data = messages_data[user_id]
                contact_messages_data.append({"name": CONNECTED_USER, "value": self.message_entry.get("1.0", "end-1c")})
                messages_data[user_id] = contact_messages_data

        with open('D:\\chat_app\\resources\\messages.json', "w") as outfile:
            outfile.write(json.dumps(messages_data, indent=4))

        self.message_entry.delete("1.0", "end")
        self.chat_messages = messages_data
        self.display_chat_messages(self.contact_id)
    ...
    def on_item_select(self, event):
        ...
        if item_id:
            contact_id = self.discussions_list.listbox_discussions.item(item_id)["values"][0]
            self.display_chat_messages(contact_id)
